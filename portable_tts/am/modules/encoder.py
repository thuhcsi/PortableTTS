...
class DurationPredictor(nn.Module):
    ...
    def __init__(self, in_channels, filter_channels, kernel_size, p_dropout):
        ...
        self.conv_1 = torch.nn.Conv1d(in_channels,
                                      filter_channels,
                                      kernel_size,
                                      padding=kernel_size // 2)
        self.norm_1 = LayerNorm(filter_channels)
        self.conv_2 = torch.nn.Conv1d(filter_channels,
                                      filter_channels,
                                      kernel_size,
                                      padding=kernel_size // 2)
...
class TextEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        torch.nn.init.normal_(self.emb.weight, 0.0, hidden_channels**-0.5)

        self.prenet = ConvReluNorm(hidden_channels,
                                   hidden_channels,
                                   hidden_channels,
                                   kernel_size=5,
                                   n_layers=3,
        ...
        self.encoder = Encoder(hidden_channels +
                               (spk_emb_dim if n_spks > 1 else 0),
                               filter_channels,
                               n_heads,
                               n_layers,
                               kernel_size,
...
class ConvReluNorm(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, kernel_size,
        ...
        for _ in range(n_layers - 1):
            self.conv_layers.append(
                torch.nn.Conv1d(hidden_channels,
                                hidden_channels,
                                kernel_size,
                                padding=kernel_size // 2))
            self.norm_layers.append(LayerNorm(hidden_channels))
