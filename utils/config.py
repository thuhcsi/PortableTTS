...
class CfgNode(dict):
    ...
    def to_flat_dict(self):
        ...
        def ctd(node, parent_name=None):
            cfg_dict = dict(node)
            for k, v in cfg_dict.items():
                if parent_name is None:
                    current_name = k
                else:
                    current_name = f'{parent_name}.{k}'
                if isinstance(v, dict):
                    ctd(v, current_name)
                else:
                    result[current_name] = str(v)
    ...
    def to_dict(self):
        ...
        def ctd(cfg_node, key_list):
            if not isinstance(cfg_node, CfgNode):
                return cfg_node
            else:
                cfg_dict = dict(cfg_node)
                for k, v in cfg_dict.items():
                    cfg_dict[k] = ctd(v, key_list + [k])
                return cfg_dict
    ...
    def dump(self, **kwargs):
        """Dump to a string."""
        self_as_dict = self.to_dict(self)
...
def from_file(path):
    with open(path) as f:
        return CfgNode._load_cfg_from_file(f)
